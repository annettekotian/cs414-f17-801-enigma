...
CREATE TABLE IF NOT EXISTS `GymSystem`.`set` (
  ...
  `exercise_machine_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  UNIQUE INDEX `number_of_reps_UNIQUE` (`repetitions` ASC),
  INDEX `fk_set_exercise1_idx` (`exercise_id` ASC, `exercise_machine_id` ASC),
  CONSTRAINT `fk_set_exercise1`
    FOREIGN KEY (`exercise_id` , `exercise_machine_id`)
    REFERENCES `GymSystem`.`exercise` (`id` , `machine_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `GymSystem`.`work_hours` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_work_hours_work_information1_idx` (`work_information_id` ASC),
  CONSTRAINT `fk_work_hours_work_information1`
